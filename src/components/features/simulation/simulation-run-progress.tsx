type SimulationRunProgressProps = {
  progress: number
}

export const SimulationRunProgress = ({ progress }: SimulationRunProgressProps) => {
  const normalizedProgress = Math.max(0, Math.min(100, progress))

  return (
    <section className="border-primary-200 bg-primary-50/85 rounded-lg border p-4">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-primary-950 text-sm font-semibold">Running simulation...</p>
      ...
      <p className="text-primary-800/75 mt-2 text-xs">
        Preparing model and computing time series. Please wait...
      </p>
    </section>
  )
}
